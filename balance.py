...
class balance(minqlbot.Plugin):
    def __init__(self):
        ...
        self.fails = 0
        # Backup of teams so we do not call teams every round on the same players
        self.teams_backup = None
        self.cascores = None
    ...
    def handle_round_start(self, round):
        minqlbot.console_print("round start.\n")
        if self.cascores is None:
            return

        teams = self.teams()
        diff = len(teams["red"]) - len(teams["blue"])

        # If team difference is more than 1
        if abs(diff) > 1:
            minqlbot.console_print("say ^7Evening teams...")
            if diff > 0:
        ...
        # If a team (still) has one more player than the other team -> move to spec
        if abs(diff) == 1:
            overplayered_team = "red"
            if len(teams["blue"]) > len(teams["red"]):
                overplayered_team = "blue"
            min_score = 999
            min_player = "999"
            for score in self.cascores:
                if score.team != overplayered_team:
                    continue # if player is in lower team
                if score.score > min_score:
                    continue # if score is higher than what we found

                min_score = score.score
                min_player = score.player.clean_name.lower()

            minqlbot.console_print("^7{} had the lowest score ({}), so (s)he is asked to spec.".format(min_player, min_score))
            self.put(min_player, "spectator")



    # added
    def handle_round_end(self, score, winner):
        # If there is a difference of X between the scores, check if the teams are still balanced.
        # We don't want this every round (6-1, 7-1, 8-1, 9-1, 10-1) since teams might not have changed
        # So store a backup of the teams when the vote was called.
        # Also don't do it when the round_end is also a game_end (i think this is a sequence)
        teams = self.teams()
        if self.game().roundlimit in score:
            minqlbot.console_print("'good game!'\n")
            minqlbot.console_command("print good game!")
            return
        elif len(teams["red"]) - len(teams["blue"]) != 0:
            # If uneven teams --> this will tell them to spec
            self.teams_info(minqlbot.CHAT_CHANNEL, self.game().short_type, True)
        elif abs(score[0]-score[1]) >= 2:
            # If even teams, check score difference
            minqlbot.console_print("^7One team is well in the lead! Checking teams for balance...")
            # there is a difference, check if we already looked at these teams:
            if self.teams_backup is None:
                self.teams_backup = deepcopy(teams)

            # Debug
            dbs = "\nOLD-red: {}\nNEW-red: {}\nOLD-blue: {}\nNEW-blue: {}\n"
            minqlbot.console_print(dbs.format(self.teams_backup['red'], teams['red'],
                                            self.teams_backup['blue'], teams['blue']))

            # if teams are still same as last time, skip suggestion
            red_team_same = teams['red'] == self.teams_backup['red']
            blue_team_same = teams['blue'] == self.teams_backup['blue']
            if red_team_same and blue_team_same:
                return

            # different teams --> call !teams
            minqlbot.console_print("Teams are different! Suggesting switch\n")
            self.teams_info(minqlbot.CHAT_CHANNEL, self.game().short_type, True)

            # update how players are aranged since check
            self.teams_backup = deepcopy(teams)
    ...
    def teams_info(self, channel, game_type, forced=False):
        ...
        if diff:
            if forced:
                # last one in team will be chosen to spec
                channel.reply("^7Last one in team will be asked to spec.")
                return True
            else:
                channel.reply("^7Both teams should have the same number of players.")
                return True
        ...
        if "Balance" in config:
            minimum_suggestion_diff = int(config["Balance"].get("MinimumSuggestionDifference", fallback="50"))
        else:
            minimum_suggestion_diff = 50
        ...
        if switch and switch[1] >= minimum_suggestion_diff:
            ...
            minqlbot.console_command("print ^7Difference {} needs to be >= than {}.".format(switch[1], minimum_suggestion_diff)) # added
