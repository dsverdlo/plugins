...
class damage(minqlbot.Plugin):
    ...
    def handle_game_end(self, game, score, winner):
        ...
        for i in range(min(len(self.scores_usefull), TOP)):
            ...
            games_completed = self.get_completed_matches(name) + 1 # cached will be 1 behind
...
##            time.sleep(1)
    ...
    def handle_scores(self, scores):
        ...
        for name in round_diffs:
            team, diff = round_diffs[name]
            tc = self.get_team_color(team)
            ...
            if team == winner and bool_matches_required:
                player_prev_top = self.db_get_top_damage_for_players(name)
        ...
        if  not SHOW_LAST_ROUND and self.game().roundlimit in self.game().scores: # TODO: add check to see if it is the last round
            return
    ...
    def cmd_top_damage(self, player, msg, channel):
        ...
        top_dmg = self.db_get_top_damage_for_players(name)
        if top_dmg < 0:
            channel.reply("^7Player ^2{}^7 has no entries in the TopDamages database table.".format(name))
        else:
            channel.reply("^7Player ^2{}^7's highest recorded damage is: ^2{}.".format(name, top_dmg))
    ...
    def cmd_alltopdmg(self, player, msg,channel):
        ...
        self.msg("^7Top damages ever recorded on bus station:")
...
##            formatted.append("^7{}^8:^3{}^8(^2{}^8)".format(i+1, dmg['name'], dmg['dmg']) + extra)
    ...
    def db_get_top_damage_for_players(self, username):
        c = self.db_query("SELECT dmg FROM TopDamages WHERE username = ? ORDER BY dmg DESC LIMIT 1",username)
        row = c.fetchone()
        if not row:
            return -1
        return row[0]
