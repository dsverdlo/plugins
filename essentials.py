...
class essentials(minqlbot.Plugin):
    ...
    def cmd_commands(self, player, msg, channel):
        commands = ['commands','time','help','seen','info','elos','elo',
            'cookies','<3','myperm','getperm','get_alias',
            'add_alias','teams','teens','checkban',
            'iou','sing','joke','dance','bbf',
            'minkyn']


        red = "^7" + ", ".join(["^7{}".format(com) for com in commands])

        channel.reply("^7My commands for you are: {}".format(red))
        # self.cmd_help(player, msg, channel)
